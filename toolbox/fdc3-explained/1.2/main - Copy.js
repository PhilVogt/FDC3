...
async function getContext(contextType) {
  try {
    ...
    if (contextType) {
      contextListener = await fdc3.addContextListener(contextType, (context) => contextResultBox.innerText = JSON.stringify(context, null, 2))
    } else {
      contextListener = await fdc3.addContextListener(null, context => contextResultBox.innerText = JSON.stringify(context, null, 2));
    }
  } catch (error) {
